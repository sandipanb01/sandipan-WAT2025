...
# Patched: fp16 + offload_folder + offload_buffers to fix OOM for large models
# + JSONL per-direction output: input_text + pred + ref
# + Training complete summary print
# ======================================================
...
class PralekhaDataset(IterableDataset):
    ...
    def __iter__(self):
        for ex in stream_examples(self.tok, self.max_samples):
            ...
            input_ids = s_enc["input_ids"] + ([self.eos_id] if self.eos_id is not None else []) + t_enc["input_ids"]
            input_ids = input_ids[:MAX_SEQ_LEN]
            src_len = len(s_enc["input_ids"]) + (1 if self.eos_id is not None else 0)
...
def evaluate_model(model, tok, max_new_tokens=256, max_samples_per_split=None, batch_size=EVAL_BATCH_SIZE):
    ...
    for split in tqdm(splits, desc="Evaluating language pairs"):
        ...
        for row in ds:
            ...
            if len(batch_prompts) >= batch_size:
                tokenized_prompts_noadd = [tok(p, add_special_tokens=False, truncation=True, max_length=MAX_SEQ_LEN) for p in batch_prompts_raw]
                input_lengths = [len(tp["input_ids"]) for tp in tokenized_prompts_noadd]
                ...
                for i, (dirn, ref, inp, prompt_len) in enumerate(zip(batch_dirs, batch_refs, batch_inputs, input_lengths)):
    ...
    for d in preds:
        ...
        # Placeholder for COMET (if implemented)
        try: comet_scores[d] = 0.0
        except: comet_scores[d] = 0.0
    ...
    for d in preds.keys():
        ...
        for i in range(min(10,len(preds[d]))):
            display(Markdown(f"**Input:** {inputs[d][i]}  \n**Ref:** {refs[d][i]}  \n**Pred:** {preds[d][i]}"))
