...
def prepare_model():
    ...
    # -------------------------------
    # ✅ SAFE MODEL LOADING FOR GEMMA ON T4 (NO META, NO AUTO DEVICE MAP)
    # -------------------------------
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_NAME,
        torch_dtype=torch.float32,
        low_cpu_mem_usage=True,
        device_map=None   # ← VERY IMPORTANT (NO META TENSORS)
    )

    model = model.to("cuda")  # ← load directly to GPU (works with fp16)
    ...
    for split in tqdm(splits, desc="Evaluating language pairs"):
        ...
        for row in ds:
            ...
            if len(batch_prompts) >= batch_size:
                ...
                for i, (dirn, ref, prompt_len) in enumerate(zip(batch_dirs, batch_refs, input_lengths)):
