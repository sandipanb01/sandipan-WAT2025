...
# Single-script with full outputs (JSONL, ZIP, metrics, plots)
# ======================================================
...
def process_batch(model, tok, batch_prompts, batch_refs, batch_dirs, batch_inputs, batch_rawlens, preds, refs, inputs, device, max_new_tokens, beam_kwargs):
    enc = tok(
        ...
        padding=True,
        truncation=True,
        max_length=max(batch_rawlens)+max_new_tokens
    )
...
def evaluate_model(model, tok, cfg):
    ...
    for d in preds:
        ...
        try: metrics[d]=sacrebleu.corpus_bleu(preds[d],[refs[d]]).score
...
def plot_training(trainer, cfg):
    ...
    if "loss" in df.columns:
        ...
        plt.figure(figsize=(8,4)); plt.plot(df["step"],df["loss"],alpha=0.5,color="gray"); plt.plot(df["step"],df["loss_smooth"],color="blue",linewidth=2)
        plt.xlabel("Step"); plt.ylabel("Loss"); plt.title("Loss over Steps"); plt.tight_layout(); plt.savefig(out_dir/"loss_smooth.png"); plt.close()
