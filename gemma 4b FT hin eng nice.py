...
class PralekhaDataset(IterableDataset):
    """Yields raw input_text and target_text. Tokenization is handled by SFTTrainer."""
    def __init__(self, tokenizer, max_samples=None):
        self.max_samples = max_samples
        self.tok = tokenizer
    def __iter__(self):
        for ex in stream_examples(self.tok, self.max_samples):
            yield {"input_text": ex["input_text"], "target_text": ex["target_text"]}
...
# ------------------------------ EVALUATION (prompt slicing kept)
def evaluate_model(model, tok, max_new_tokens=256, max_samples_per_split=None, batch_size=EVAL_BATCH_SIZE):
    ...
    # JSONL & ZIP output + metrics + plots are unchanged
    # ------------------------------
    for d in preds:
