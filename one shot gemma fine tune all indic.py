    ...
    for split in splits:
        ...
        for row in ds:
            ...
            for s_txt,t_txt,dirn in [(eng,indic,f"eng_{lang}"),(indic,eng,f"{lang}_eng")]:
                # Only include example if training, skip placeholder in evaluation
...
def evaluate_model(model, tok, max_new_tokens=256, max_samples_per_split=None, batch_size=EVAL_BATCH_SIZE):
    ...
    for split in tqdm(splits, desc="Evaluating language pairs"):
        ...
        for row in ds:
            ...
            if not s or not t: continue
            eng, indic = (s,t) if sl=="eng" else (t,s)
            # Use empty example to prevent English bias
            batch_prompts += [
                build_prompt(eng,"eng",lang,("", ""),tok),
                build_prompt(indic,lang,"eng",("", ""),tok)
            ]
...
# ------------------------------ BLEU/chrF Metric Plots (Colab-friendly)
