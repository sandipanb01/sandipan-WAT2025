...
# ============================================================
# SEPARATE CELL: BASELINE EVALUATION SCORES TO EXCEL
# ============================================================
import pandas as pd
import sacrebleu
import numpy as np
...
def export_baseline_report(results_list, output_folder):
    """
    Calculates metrics for the baseline results and exports to Excel.
    """
    ...
    for mode in directions:
        ...
        for p in preds:
            ...
            is_correct = has_hin if mode == "ENG_to_HIN" else not has_hin
            lid_hits.append(is_correct)
        
        summary_stats.append({
            "Direction": mode.replace("_", " "),
            "BLEU": round(bleu, 2),
            "chrF": round(chrf, 2),
            "Script Accuracy (%)": round(np.mean(lid_hits) * 100, 2),
            "Samples": len(subset),
            "Model Type": "Baseline (Zero-Shot)"
        })

    # Create Excel report
    report_df = pd.DataFrame(summary_stats)
    os.makedirs(output_folder, exist_ok=True)
    excel_path = os.path.join(output_folder, "baseline_metrics_report.xlsx")
    
    report_df.to_excel(excel_path, index=False)
    
    print("\n" + "═"*65)
    print(f"✅ BASELINE EXCEL CREATED: {excel_path}")
    print("═"*65)
    print(report_df.to_string(index=False))
    print("═"*65)

# Run the export using the results from the baseline loop
export_baseline_report(results, OUTPUT_DIR)
