...
def load_streaming_dataset(tokenizer, split="train", max_samples=100, one_shot=True):
    ...
    for row in ds:
        ...
        # Always create BOTH directions: eng→hin and hin→eng
        eng, hin = (src_txt, tgt_txt) if row["src_lang"] == "eng" else (tgt_txt, src_txt)

        for src, tgt, direction in [(eng, hin, "eng_hin"), (hin, eng, "hin_eng")]:
