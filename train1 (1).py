...
raw = raw.map(clean_utf8, desc="Decoding UTF-8")

# --- STEP 3: Cast Back to String (CRITICAL FIX) ---
# This forces the dataset schema to recognize these columns as strings again
...
# Decoding corrupt data might result in empty strings; remove them.
raw = raw.filter(lambda x: x["src_txt"] and len(x["src_txt"].strip()) > 0, desc="Removing empty src")
raw = raw.filter(lambda x: x["tgt_txt"] and len(x["tgt_txt"].strip()) > 0, desc="Removing empty tgt")

print("4. Applying logic filters...")
raw = raw.filter(strict_filter, desc="Strict Filter")
