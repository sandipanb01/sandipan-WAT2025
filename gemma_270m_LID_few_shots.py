# ============================================================
# Gemma-3-270M-IT FEW-SHOT MT BASELINE 
# I guess this prevents "Yapping" and "Source Copying" via Few-Shots but still 270m is too small to learn meaningful translation even with few shots.
# ============================================================
...
MAX_SAMPLES = 10                  
MAX_NEW_TOKENS = 512              # Reduced to prevent long-winded hallucination
BATCH_SIZE = 1                    
...
def check_translation_validity(pred_text: str, target_lang_code: str) -> int:
    if not pred_text.strip(): return 0
    try:
...
def make_prompt(src_text: str, src_lang: str, tgt_lang: str) -> str:
    """
    Few-shot prompt to anchor the 270M model.
    Pre-fills the model response to prevent English preamble.
    """
    if src_lang.lower() == "eng":
        examples = (
            "Translate from ENG to HIN:\n"
...
def translate(tokenizer, model, prompts: List[str]) -> List[str]:
    ...
    for prompt in prompts:
        ...
        # Cleanup: Remove potential leftover labels if model hallucinates them
        text = text.split("English:")[0].split("Hindi:")[0].strip()
...
def evaluate_direction(tokenizer, model, src_lang, tgt_lang, src_texts, ref_texts, tag):
    ...
    print(f"--- Running {tag} (Few-Shot) ---")
    for i in tqdm(range(0, len(prompts), BATCH_SIZE)):
        predictions.extend(translate(tokenizer, model, prompts[i:i + BATCH_SIZE]))
    ...
    df = pd.DataFrame({"src": src_texts, "ref": ref_texts, "pred": predictions, "lang_correct": valid_hits})
...
def main():
    ...
    for direction in DIRECTIONS:
        ...
        if MAX_SAMPLES: ds = ds.select(range(min(len(ds), MAX_SAMPLES)))

        # ENG -> HIN
        bleu, chrf, l_ratio = evaluate_direction(tokenizer, model, src, tgt, ds["src_txt"], ds["tgt_txt"], f"{src}_{tgt}_test")
        results.append({"direction": f"{src}_{tgt}", "BLEU": bleu, "ChrF": chrf, "LID_Accuracy": l_ratio})

        # HIN -> ENG
        bleu_r, chrf_r, l_ratio_r = evaluate_direction(tokenizer, model, tgt, src, ds["tgt_txt"], ds["src_txt"], f"{tgt}_{src}_test")
